...
/*---------------------------------------------------------------------------*/
int
bsp_hw_mpu9250_dmp_open(void *args)
{
  if (m_mpu9250_active) {
    return EINVALSTATE;
  }
...
/*---------------------------------------------------------------------------*/
int
bsp_hw_mpu9250_dmp_write(const void *buf, uint32_t len, uint32_t *offset)
{
  return ENOSUPPORT;
}
/*---------------------------------------------------------------------------*/
int
bsp_hw_mpu9250_dmp_read(void *buf, uint32_t len, uint32_t offset)
{
  switch (offset) {
    case 0:
    {
...
/*---------------------------------------------------------------------------*/
int
bsp_hw_mpu9250_dmp_subscribe(void *buf, uint32_t len, HWCallbackHandler handler)
{
  return ENOSUPPORT;
}
/*---------------------------------------------------------------------------*/
int
bsp_hw_mpu9250_dmp_close(void *args)
{
  if (!m_mpu9250_active) {
    return EINVALSTATE;
  }
