...
/* AlfaOne Sensor Heading UUID {7943000A-F6C2-09A3-E9F9-128ABCA31297} */
const BleUuid16 gatt_alfaone_sensor_heading_chr_uuid = BLE_UUID16_INIT(0x000A);
/* AlfaOne Notify Control Pointer UUID {7943000B-F6C2-09A3-E9F9-128ABCA31297} */
const BleUuid16 gatt_alfaone_notify_control_chr_uuid = BLE_UUID16_INIT(0x000B);
/*---------------------------------------------------------------------------*/
...
static uint16_t ble_attr_notify_control_handle;
static uint16_t ble_attr_notify_control_cccd_handle;
/*---------------------------------------------------------------------------*/
static uint8_t ieefp4_tx_buffer[13];
static uint8_t accel_tx_buffer[17];
static uint8_t gyro_tx_buffer[17];
static uint8_t compass_tx_buffer[17];
static uint8_t quat_tx_buffer[20];
static uint8_t euler_tx_buffer[17];
static uint8_t linear_accel_tx_buffer[17];
static uint8_t gravity_vector_tx_buffer[17];
static uint8_t heading_tx_buffer[9];
static uint8_t notify_control_buffer[9];
/*---------------------------------------------------------------------------*/
...
static BleGattService g_ble_gatt_alfa_ibeacon_service = {
  ...
  .characteristics = (BleGattCharacteristic[]) { {
    ...
    .uuid = &gatt_alfaone_notify_control_chr_uuid,
    .value_handle = &ble_attr_notify_control_handle,
    .cccd_handle = &ble_attr_notify_control_cccd_handle,
    .props = BLE_GATT_CHR_PROPS_WRITE | BLE_GATT_CHR_PROPS_WRITE_NO_RSP,
    .permission = BLE_GATT_CHR_PERMISSION_READ | BLE_GATT_CHR_PERMISSION_WRITE,
    .init_value = &notify_control_buffer[0],
    .init_value_len = 9,
  }, {
...
{
	if (handle == ble_attr_notify_control_handle && length == 9) {
		logger->printf(LOG_RTT, "Set notify_control_buffer\n");
		memcpy(notify_control_buffer, value, 9);
	}
...
ble_gap_conn_evt_handler(uint16_t conn_handle, uint16_t state)
{
  ...
  if (state == BLE_GATT_STATE_CONNECTED) {
    ...
    is_connected = true;
	memset(notify_control_buffer, 0, 9);
...
{
  int errcode;
  bool notify_enabled_flag = false;
    ...
    if (timer_flag) {
      ...
      if (is_connected) {
        ...
        ieefp4->read(&ieefp4_data_inst, sizeof(ieefp4_data_t), 0);
				...
				// notify
        if (is_ieefp4_notify_enabled && notify_control_buffer[0]) {
			notify_enabled_flag = true;
			ieefp4_tx_buffer[12] = sequence;
			ble_manager->notifyCharacteristic(peripheral_conn_handle, ble_attr_ieefp4_handle, ieefp4_tx_buffer, 13);
		}
		
        if (is_accel_notify_enabled && notify_control_buffer[1]) {
			notify_enabled_flag = true;
			accel_tx_buffer[16] = sequence;
			ble_manager->notifyCharacteristic(peripheral_conn_handle, ble_attr_accel_handle, accel_tx_buffer, 17);
		}
        if (is_gyro_notify_enabled && notify_control_buffer[2]) {
			notify_enabled_flag = true;
			gyro_tx_buffer[16] = sequence;
			ble_manager->notifyCharacteristic(peripheral_conn_handle, ble_attr_gyro_handle, gyro_tx_buffer, 17);
		}
		if (is_compass_notify_enabled && notify_control_buffer[3]) {
			notify_enabled_flag = true;
			compass_tx_buffer[16] = sequence;
			ble_manager->notifyCharacteristic(peripheral_conn_handle, ble_attr_compass_handle, compass_tx_buffer, 17);
		}
		if (is_quat_notify_enabled && notify_control_buffer[4]) {
			notify_enabled_flag = true;
			ble_manager->notifyCharacteristic(peripheral_conn_handle, ble_attr_quat_handle, quat_tx_buffer, 20);
		}
		if (is_euler_notify_enabled && notify_control_buffer[5]) {
			notify_enabled_flag = true;
			euler_tx_buffer[16] = sequence;
			ble_manager->notifyCharacteristic(peripheral_conn_handle, ble_attr_euler_handle, euler_tx_buffer, 17);
		}
		if (is_linear_accel_notify_enabled && notify_control_buffer[6]) {
			notify_enabled_flag = true;
			linear_accel_tx_buffer[16] = sequence;
			ble_manager->notifyCharacteristic(peripheral_conn_handle, ble_attr_linear_accel_handle, linear_accel_tx_buffer, 17);
		}
		if (is_gravity_vector_notify_enabled && notify_control_buffer[7]) {
			notify_enabled_flag = true;
			gravity_vector_tx_buffer[16] = sequence;
			ble_manager->notifyCharacteristic(peripheral_conn_handle, ble_attr_gravity_vector_handle, gravity_vector_tx_buffer, 17);
		}
		if (is_heading_notify_enabled && notify_control_buffer[8]) {
			notify_enabled_flag = true;
			heading_tx_buffer[8] = sequence;
			ble_manager->notifyCharacteristic(peripheral_conn_handle, ble_attr_heading_handle, heading_tx_buffer, 9);
		}
