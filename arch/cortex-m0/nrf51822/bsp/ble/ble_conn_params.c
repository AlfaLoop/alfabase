...
static struct ctimer m_conn_update_ctimer;		 							 /**< Connection parameters timer. */
static bool m_change_param = false;
/*---------------------------------------------------------------------------*/
static bool
is_conn_params_ok(ble_gap_conn_params_t * p_conn_params)
{
            ...
            if (m_conn_params_config.disconnect_on_fail)
            {
                ...
                if ((err_code != ENONE) && (m_conn_params_config.error_handler != NULL))
                {
                    ...
                    m_conn_params_config.error_handler(err_code);
				        ...
				        PRINTF("[ble conn params] Negotiation failed\n");
            }
    ...
    if (!is_conn_params_ok(&m_current_conn_params))
    {
        ...
        else
        {
            if (m_update_count == 0)
            {
                ...
                timeout_sec = m_conn_params_config.first_conn_params_update_delay;
            }
            else
            {
                timeout_sec = m_conn_params_config.next_conn_params_update_delay;
            }

		   	ctimer_set(&m_conn_update_ctimer, timeout_sec , update_timeout_handler, (void *)NULL);
...
/*---------------------------------------------------------------------------*/
static void
on_connect(ble_evt_t * p_ble_evt)
{
...
/*---------------------------------------------------------------------------*/
static void
on_disconnect(ble_evt_t * p_ble_evt)
{
    uint32_t err_code;
	...
	ctimer_stop(&m_conn_update_ctimer);
	if ( (m_conn_params_config.error_handler != NULL))
    {
        m_conn_params_config.error_handler(err_code);
    }
...
/*---------------------------------------------------------------------------*/
static void
on_conn_params_update(ble_evt_t * p_ble_evt)
{
    // Copy the parameters
...
/*---------------------------------------------------------------------------*/
uint32_t
ble_conn_params_init(const ble_conn_params_init_t * p_init)
{
    uint32_t err_code;
    ...
    if (p_init->p_conn_params != NULL)
    {
        ...
        {
			PRINTF("sd_ble_gap_ppcp_set error %d\n", err_code);
            return err_code;
        }
    }
    else
    {
        ...
        if (err_code != NRF_SUCCESS)
        {
			PRINTF("sd_ble_gap_ppcp_get error %d\n", err_code);
            return err_code;
        }
    }
