                                 ...
                                 softdevice_evt_schedule_func_t evt_schedule_func)
{
    ...
    PRINTF("[softdevice_handler] sd_softdevice_enable error %d\n", err_code);
#endif
...
uint32_t sd_check_ram_start(uint32_t sd_req_ram_start)
{
    ...
    {
        PRINTF("RAM START ADDR 0x%x should be adjusted to 0x%x\r\n",
                  ram_start,
                  sd_req_ram_start);
        PRINTF("RAM SIZE should be adjusted to 0x%x \r\n",
...
uint32_t softdevice_enable(ble_enable_params_t * p_ble_enable_params)
{
    ...
    PRINTF("[softdevice_handler] softdevice_enable\n");
    ...
    {
        PRINTF("sd_ble_enable: app_ram_base should be adjusted to 0x%x\n",
                app_ram_base);
        PRINTF("ram size should be adjusted to 0x%x\n",
