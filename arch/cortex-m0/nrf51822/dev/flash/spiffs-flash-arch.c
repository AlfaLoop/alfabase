...
w25q_flash_read_status(uint8_t *status)
{
  ...
  spi0_driver.transfer(command, 1, NULL, 0);
	spi0_driver.transfer(NULL, 0, status, 1);
...
w25q_flash_write_enable(void)
{
	...
	spi0_driver.enable();
	nrf_gpio_pin_clear(SPI_FLASH_CS);
	errcode = spi0_driver.transfer(command, 1, NULL, 0);
	nrf_gpio_pin_set(SPI_FLASH_CS);
	spi0_driver.disable();
	if (errcode != ENONE)
		return false;
...
w25q_flash_write_page(uint32_t addr, uint8_t *data, uint32_t length)
{
  bool success = w25q_flash_write_enable();
	if (success) {
		...
		spi0_driver.enable();
		nrf_gpio_pin_clear(SPI_FLASH_CS);
		spi0_driver.transfer(command, 4, NULL, 0);
		spi0_driver.transfer(data, length, NULL, 0);
    watchdog_periodic();
		nrf_gpio_pin_set(SPI_FLASH_CS);
		spi0_driver.disable();
	}
...
w25q_flash_erase_sector(uint32_t addr)
{
	...
	if (success) {
		...
		spi0_driver.enable();
		nrf_gpio_pin_clear(SPI_FLASH_CS);
		errcode = spi0_driver.transfer(command, 4, NULL, 0);
		nrf_gpio_pin_set(SPI_FLASH_CS);
		spi0_driver.disable();
		if (errcode != ENONE) {
			success = false;
...
spiffs_w25q_read(struct spiffs_t *fs, u32_t addr, u32_t size, u8_t *dst)
{
  ...
  watchdog_periodic();
	...
	nrf_gpio_pin_clear(SPI_FLASH_CS);
	errcode = spi0_driver.transfer(command, 4, NULL, 0);
	errcode = spi0_driver.transfer(NULL, 0, dst, size);
	nrf_gpio_pin_set(SPI_FLASH_CS);
	spi0_driver.disable();
...
spiffs_flash_arch_init(uint8_t format)
{
  ...
  spi0_driver.enable();
  nrf_gpio_pin_clear(SPI_FLASH_CS);
  errcode = spi0_driver.transfer(&command[0], 4, NULL, 0);
  errcode = spi0_driver.transfer(NULL, 0, result, 2);
  nrf_gpio_pin_set(SPI_FLASH_CS);
  spi0_driver.disable();
