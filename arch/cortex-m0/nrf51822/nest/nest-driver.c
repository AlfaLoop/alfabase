...
on_ble_central_evt(const ble_evt_t * const p_ble_evt)
{
  ...
  switch (p_ble_evt->header.evt_id)
  {
    ...
    /*  PRINTF("[nest driver] scanned device: type:%d %02x:%02x:%02x:%02x:%02x:%02x rssi: %d\n",
...
on_ble_peripheral_evt(ble_evt_t * p_ble_evt)
{
  ...
  switch (p_ble_evt->header.evt_id)
  {
    ...
    case BLE_GATTS_EVT_SYS_ATTR_MISSING:
    {
      ...
      if (err_code != NRF_SUCCESS) {
        PRINTF("sd_ble_gatts_sys_attr_set error:%d\n", err_code);
      }
    }
    ...
    case BLE_GAP_EVT_SEC_PARAMS_REQUEST:
      ...
      err_code = sd_ble_gap_sec_params_reply(p_ble_evt->evt.gap_evt.conn_handle, BLE_GAP_SEC_STATUS_PAIRING_NOT_SUPP, NULL, NULL);
			if (err_code != NRF_SUCCESS) {
				PRINTF("sd_ble_gap_sec_params_reply error:%d\n", err_code);
			}
		}
    break;
...
nrf_ble_reset(void)
{
  ...
  err_code = ble_radio_notification_init(3,
                                         NRF_RADIO_NOTIFICATION_DISTANCE_800US,
                                         ble_on_radio_active_evt);
  if (err_code != NRF_SUCCESS){
    PRINTF("[nest driver] ble_radio_notification_init %d\n", err_code);
    return EINVAL;
  }
  ...
  PRINTF("[nest driver] reset completed\n");
}
/*---------------------------------------------------------------------------*/
static int
init(void)
