...
/*---------------------------------------------------------------------------*/
static int
i2chw0_init(const i2c_config_t *config)
{
...
/*---------------------------------------------------------------------------*/
static int
i2chw0_transfer(uint8_t type, uint8_t address, uint32_t length, uint8_t * p_data, uint8_t pending)
{
	uint32_t err_code = ENONE;
	if (type == I2C_TX) {
		if (pending) {
...
/*---------------------------------------------------------------------------*/
const struct i2c_driver nrf_twi_hw_driver0 = {
	"i2chw0",
	i2chw0_init,
	i2chw0_enable,
	i2chw0_disable,
