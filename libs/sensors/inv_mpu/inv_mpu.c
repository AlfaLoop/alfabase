...
static int
i2cwrite(uint8_t address, uint8_t reg, uint32_t length, uint8_t *p_data)
{
	int err_code;
	uint8_t tx[16];
	memset(tx, 0x00, sizeof(tx));
	tx[0] = reg;
	memcpy(&tx[1], p_data, length);
	TWI.enable();
	err_code = TWI.transfer(I2C_TX, address, length + 1, tx, I2C_DONT_PEDDING);
	TWI.disable();
...
static int
i2cread(uint8_t address, uint8_t reg, uint32_t length, uint8_t * p_data)
{
	int err_code;

	TWI.enable();
	err_code = TWI.transfer(I2C_TX, address, 1, &reg, I2C_PEDDING);
	TWI.disable();

//		PRINTF("i2cread addr:%x reg:%x length:%d err:%d\n", address, reg, length, err_code);
  if (err_code) {
		return err_code;
	}

	TWI.enable();
	err_code = TWI.transfer(I2C_RX, address, length, p_data, I2C_DONT_PEDDING);
	TWI.disable();
...
static uint32_t
get_timestamp(uint32_t *timestamp)
{
...
#ifdef AK89xx_SECONDARY
    int compass_err = setup_compass();
			PRINTF("setup_compass. %d\n", compass_err);
    if (mpu_set_compass_sample_rate(10))
        return -1;
#else
		PRINTF("Already disabled by setup_compass.\n");
...
int mpu_set_int_data_ready(unsigned char enable)
{
...
static int setup_compass(void)
{
    ...
    mpu_set_bypass(1);
		...
		/* Find compass. Possible addresses range from 0x0C to 0x0F.(0x18 to 0x1b) */
		/********************   IMPORTANT    **********************/
		/* NRF I2C library need 8-bit address of the device to communicate.
    ...
    if (akm_addr > (0x0F)<<1) {
        ...
        log_e("Compass not found.%d\n", akm_addr);
        return -1;
    }
		else{
			  log_i("Compass found. %d\n", akm_addr);
		}
...
#if INV_MPU_SELF_TEST_ENABLE_CONF == 1
/**
...
#endif // INV_MPU_SELF_TEST_ENABLE_CONF
